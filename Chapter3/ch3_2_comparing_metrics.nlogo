globals [
  ...
  unhappiness  ;; proportion of the agents that are unhappy
  prop-uniform ;; proportion of the agents that have zero neighbors of the opposite color
]

turtles-own [
happy? ;; enough neighbors like me?
prop-similar-neighbors ;; proportion similar neighbors
uniform? ;; agents that have zero neighbors of the opposite color
]
...
to make-turtles
  ifelse exact? [
    let population density * count (patches)
  ...
  ask turtles [
   ifelse (neighborhood-type = "von Neumann")
      [let nearby von-neumann-offsets radius
       ...
       let similar-nearby count (turtles-on neighborhood) with
          [color = [color] of myself]
       let opposite-nearby count (turtles-on neighborhood) with
          [color != [color] of myself]
       let total-nearby count (turtles-on neighborhood)
       ifelse (total-nearby = 0 and happy-alone?) ;; switch determines if no-neighbors is reason to move
              [set prop-similar-neighbors 1]
              [set prop-similar-neighbors 0]
       if (total-nearby > 0)
       [set prop-similar-neighbors (similar-nearby / total-nearby)]
       set happy? (prop-similar-neighbors >= similarity-threshold)
       set uniform? opposite-nearby = 0]
...
to move-unhappy-turtles
  ask turtles with [not happy?] [
    ;; the nearest empty patch
    let lazy-choice min-one-of other patches with [not any? turtles-here] [distance myself]

    (ifelse
    (moving-strategy = "anywhere") [move-to one-of patches with [not any? turtles-here]]
    (moving-strategy = "lazy")     [move-to lazy-choice])
  ]
end
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot unhappiness"
